...
const CommandOption = ({ placeholder, options, onClickItems, clearTags, applyTags, selectedOptions, type } : Props) => {
    const [searchTerm, setSearchTerm] = useState<string>("");
    // Gunakan useEffect untuk mensinkronkan compOptions jika 'options' berubah dari parent
    const [compOptions, setCompOptions] = useState(options);

    // useEffect(() => {
    //     setCompOptions(options);
    //     // Reset search term when options change, or if you want to keep it, adjust
    //     setSearchTerm("");
    // }, [options]);
    ...
    const filterOptions = (newSearchTerm:string) => {
        ...
        const newOps = options.filter(option => (option.toLowerCase().includes(newSearchTerm.toLowerCase()) || !newSearchTerm));
        setCompOptions(newOps);
    }
